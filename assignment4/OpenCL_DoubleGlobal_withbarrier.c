...
int main(int argc,const char * argv[])
{
  ...
  error = clGetPlatformIDs(2, platform_id, &num_platforms);
  if(error != CL_SUCCESS)
  {
  ...
  cl_uint num_devices;
  if(clGetDeviceIDs(platform_id[1], CL_DEVICE_TYPE_CPU, 1, &device_id, &num_devices) != CL_SUCCESS)
  {
  ...
  */
  ...
  FILE *fp;
  fp = fopen("compute_temperatures_doubleglobal.cl", "r");
  if (!fp)
  {
  ...
  cl_mem told_mem_obj = clCreateBuffer(context, CL_MEM_READ_WRITE, length * width * sizeof(double), NULL, &error);
  ...
  clSetKernelArg(kernel, 1, sizeof(int), &length);
  ...
  // Read the memory buffer told on the device to the local variable told
  clEnqueueReadBuffer(command_queue, told_mem_obj, CL_TRUE, 0, length * width * sizeof(double), told , 0, NULL, NULL);
