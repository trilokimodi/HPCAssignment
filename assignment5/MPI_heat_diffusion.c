...
main(int argc, char **argv)
{
  ...
  if (mpi_rank == 0) 
  {
    ...
    for(int iter = 0; iter<num_iterations/2;++iter)
    {
      ...
      if(flag > 0)
      {
        lastpositiontoworker = (dest-1)*numelems2copymaster;
        printf("my position to work %d\n",lastpositiontoworker+1);
...
printf("Actual ans for -n2 -d0.01 100*100 is 486,1699. Our counts = %d\n",counts);
  ...
  if (mpi_rank > 0)
  {
    ...
    double * tnew1 = (double *)malloc(sizeof(double) * length*width);
    for(int i=0; i<length*width; ++i)
      tnew1[i] = 0;
    ...
    for(int i=k,tnew_line=0; count < numele2change; i+=width,++tnew_line)
    {
      for(j=1; j<width-1; ++j)
      {        
        tnew1[tnew_line*width+k+j-1] = told1[tnew_line*width + k + j-1] + dc * (((told1[tnew_line*width+k+j] + told1[(tnew_line+1)*width+k+j-1] + told1[(tnew_line-1)*width + k+j-1] + told1[tnew_line*width+k-1+j-1])/4) - told1[tnew_line*width+k+j-1]);
        ++count;
      }
    }

    MPI_Send(&startingposition, 1, MPI_INT, 0, 8, MPI_COMM_WORLD);
    MPI_Send(&tnew1[startingposition], numeletnew, MPI_DOUBLE, 0, 9, MPI_COMM_WORLD);
    free(told1);
    free(tnew1);
  }
  MPI_Finalize();
